...
DEFAULT_STUB = TEST_DATA_DIR / "nrfutil.py"

ENV_NRFUTIL_PATH = "NRFUTIL_PATH"
...
class NrfutilStub:
    ...
    NRFUTIL_PATH selects another stub, which has to follow the same contract as
    :file:`nrfutil.py`. A stub without the executable bit is run with the Python
    ...
    def __init__(self, root: Path):
        ...
        stub = Path(os.environ.get(ENV_NRFUTIL_PATH, DEFAULT_STUB)).resolve()
        if not stub.is_file():
            raise FileNotFoundError(
                f"{ENV_NRFUTIL_PATH} is not a file: {stub}")

        self.bin_dir = root / "bin"
        self.bin_dir.mkdir()
        executable = self.bin_dir / "nrfutil"
        if os.access(stub, os.X_OK):
            executable.symlink_to(stub)
        else:
