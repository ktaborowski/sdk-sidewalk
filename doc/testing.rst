...
The default stub is the :file:`test_data/nrfutil.py` script, which reports a connected J-Link device instead of listing the real ones, and keeps a copy of the HEX file instead of programming it.
...
To use a different stub, pass its path in the ``NRFUTIL_PATH`` environment variable:

.. code-block:: bash

   NRFUTIL_PATH=path/to/stub/nrfutil python3 -m unittest discover --start-directory sidewalk/scripts/west_commands/tests

The stub must follow the same contract as :file:`test_data/nrfutil.py`.
A stub without the executable permission is run with the Python interpreter of the tests.
